...
import { Train, BarChart3, Settings, ClipboardList, RefreshCw, Upload } from 'lucide-react'
import TaskTracker from './components/TaskTracker'
import EfficiencyDashboard from './components/EfficiencyDashboard'
import AdminPanel from './components/AdminPanel'
import './App.css'

function NavBar({ isRefreshing, onRefresh }) {
  const location = useLocation()
  const isTaskTrackerPage = location.pathname === '/'

  const handleUploadClick = () => {
    // Dispatch event for TaskTracker to open file dialog
    window.dispatchEvent(new CustomEvent('triggerExcelUpload'))
  }
...
function App() {
  ...
  return (
    <BrowserRouter>
      <div className="app">
        <NavBar isRefreshing={isRefreshing} onRefresh={handleRefresh} />
